...
static mini_slot_t s_max7219_pool_ctrl MINI_ALIGNED(4);
...
{
    MINI_IGNORE_RESULT(mini_slot_init(&s_max7219_pool_ctrl, s_max7219_used, MAX7219_POOL_COUNT));
}
...
static int max7219_probe(struct device* pdev)
{
    ...
    if (!pdev)
        return MINI_ERR_INVAL;
    pool_idx = mini_slot_claim(&s_max7219_pool_ctrl);
    if (pool_idx < 0)
...
err:
    pdev->ops = NULL;
    MINI_MEM_SET(dev, 0, sizeof(*dev));
    MINI_IGNORE_RESULT(mini_slot_release(&s_max7219_pool_ctrl, pool_idx));
    return ret;
}
...
static int max7219_remove(struct device* pdev)
{
    ...
    max7219_hw_destroy(dev);
    MINI_MEM_SET(dev, 0, sizeof(*dev));
    MINI_IGNORE_RESULT(mini_slot_release(&s_max7219_pool_ctrl, idx));
    dev_lc_remove_finish(lc);
    return MINI_OK;
}
