...
static mini_slot_t s_xpt2046_pool_ctrl MINI_ALIGNED(4);
...
{
    MINI_IGNORE_RESULT(mini_slot_init(&s_xpt2046_pool_ctrl, s_xpt2046_used, XPT2046_POOL_COUNT));
}
...
static int xpt2046_probe(struct device* pdev)
{
    ...
    if (!pdev)
        return MINI_ERR_INVAL;
    pool_idx = mini_slot_claim(&s_xpt2046_pool_ctrl);
    if (pool_idx < 0)
...
err:
    pdev->ops = NULL;
    MINI_MEM_SET(dev, 0, sizeof(*dev));
    MINI_IGNORE_RESULT(mini_slot_release(&s_xpt2046_pool_ctrl, pool_idx));
    return ret;
}
...
static int xpt2046_remove(struct device* pdev)
{
    ...
    xpt2046_hw_destroy(dev);
    MINI_MEM_SET(dev, 0, sizeof(*dev));
    MINI_IGNORE_RESULT(mini_slot_release(&s_xpt2046_pool_ctrl, idx));
    dev_lc_remove_finish(lc);
    return MINI_OK;
}
