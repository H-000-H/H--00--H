...
static mini_slot_t s_ssd1306_pool_ctrl MINI_ALIGNED(4);
...
{
    MINI_IGNORE_RESULT(mini_slot_init(&s_ssd1306_pool_ctrl, s_ssd1306_used, SSD1306_POOL_COUNT));
}
...
static int ssd1306_cmd_draw_area(struct ssd1306_device* dev, void* arg, size_t len, uint32_t ms)
{
    ...
    for (page = 0; page < SSD1306_PAGES; page++)
    {
        ...
        while (off < SSD1306_WIDTH)
        {
            ...
            if (chunk_len > 64U)
                chunk_len = 64U;
            MINI_IGNORE_RESULT(MINI_MEM_COPY(&chunk[1], &darg->data[page * SSD1306_WIDTH + off], chunk_len));
...
static int ssd1306_probe(struct device* pdev)
{
    ...
    if (!pdev)
        return MINI_ERR_INVAL;
    pool_idx = mini_slot_claim(&s_ssd1306_pool_ctrl);
    if (pool_idx < 0)
...
err:
    pdev->ops = NULL;
    MINI_MEM_SET(dev, 0, sizeof(*dev));
    MINI_IGNORE_RESULT(mini_slot_release(&s_ssd1306_pool_ctrl, pool_idx));
    return ret;
}
...
static int ssd1306_remove(struct device* pdev)
{
    ...
    ssd1306_hw_destroy(dev);
    MINI_MEM_SET(dev, 0, sizeof(*dev));
    MINI_IGNORE_RESULT(mini_slot_release(&s_ssd1306_pool_ctrl, idx));
    dev_lc_remove_finish(lc);
    return MINI_OK;
}
