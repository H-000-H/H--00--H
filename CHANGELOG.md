...
- **裸机 `mini_malloc/mini_calloc/mini_free` 可选切换到 mini-os 内存模块**：新增 `（已移除: 内存统一走 libc/内核堆）`（默认关，关闭时维持 libc `malloc/calloc/free`）与公共 API `mini_malloc()`（四后端同步提供）。开启后只编入 `lib/mini-os/src/memory.c` 单文件（无调度器/port 依赖，不链整个内核），`mini_malloc/mini_calloc/mini_free` 转发到 mini-os 堆 —— first-fit + 相邻合并 + magic 防 double-free，堆区由链接脚本 `__mini_os_heap_start/__mini_os_heap_end` 提供（可 `INCLUDE mini-os-heap.ld`），首次分配惰性接管（`mini_os_heap_ensure_init()` 幂等），无需启动遍历 `.init_array`；空闲链表无锁，ISR 内禁止调用。
  **Bare-metal `mini_malloc/mini_calloc/mini_free` can optionally switch to the mini-os memory module**: new `（已移除: 内存统一走 libc/内核堆）` (default off — keeps libc `malloc/calloc/free`) and the public `mini_malloc()` API (provided by all four backends). When on, only `lib/mini-os/src/memory.c` is compiled in (no scheduler/port dependency, the whole kernel is not linked) and `mini_malloc/mini_calloc/mini_free` forward to the mini-os heap — first-fit + adjacent coalescing + magic double-free guard; the heap zone comes from the linker symbols `__mini_os_heap_start/__mini_os_heap_end` (`INCLUDE mini-os-heap.ld`), taken over lazily on the first allocation (idempotent `mini_os_heap_ensure_init()`), no `.init_array` traversal needed; the free list is unlocked — never call from an ISR.
