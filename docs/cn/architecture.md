...
横向: core · interrupt · system_c|cpp · time_slice · can_hook · net · ui · tools
...
| `lib/` + `cmake/*.cmake` | vendor：mini-os / FreeRTOS / RT-Thread / ETL；TinyUSB / lwIP 为配置期 FetchContent，其余积木链接期 FetchContent | OS 内核按 Kconfig；其余 `mini_tree_link_*`（见 [ecosystem.md](ecosystem.md)） |
...
`algorithm/buffer` 提供环形/双缓冲等结构，供驱动与业务复用。
...
Kconfig 菜单：Platform、Multi-core/AMP、OS 后端、Spinlock、System Log、System Runtime（`SYSTEM` 总开关 + C/CPP 后端）、Components（USB）、Board Features（WDT/Scrubber/…）、Runtime Capacity（`EVENT_BUS` 总开关 + 容量）、Compiler、Build。
...
| OS 四后端骨架 | 时钟、堆、SysTick/RTOS 端口（若需要） |
