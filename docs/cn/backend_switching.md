...
裸机后端 (`CONFIG_OS_BARE`) 的任务调度器由 `Kconfig.mini_tree` 的「裸机调度器」choice 三选一（`XTASK_NONE` / `XTASK_COOP` / `XTASK_PREEMPT`）, CMake 据 `.config` 注入 `MINI_TREE_XTASK_*` 宏决定编译 `xtask_coop.c` 或 `xtask_preempt.c`, 源码 `#ifdef` 双重互斥:
- **无调度**（`XTASK_NONE`）— 不编入任何调度器, 应用层自写 `while(1)` 大循环; `CONFIG_XTASK_PREEMPT` 由 Kconfig 自动关闭, core/system 层依赖 xtask 接口无法链接, 固件退化为裸闭包.
...
1. 改 `mini_tree/.config` 中 OS choice（互斥）。
...
- 裸机 C API `mini_task_create` / `mini_task_create_handle` 恒返回 `MINI_ERR_NOTSUPP`.

> **抢占式开启 (`XTASK_PREEMPT=y`) 时**: C++ 重载仍提供, 但签名切换为带 `priority` 的分支（`stack_size` 复用为周期）; 协调式与抢占式分支在 `（C++ 封装已移除）` 内以 `CONFIG_XTASK_PREEMPT` 区分, 无需业务改调用. 也可走 `XTASK_NONE` 直接裸写 `while` 循环.
...
- 只使用 `mini_backend.h` 中标明可用于 ISR 的 API（若有）；不确定则假设 **不可** 在 ISR 拿 mutex。
- Spinlock 实现由 `CONFIG_MINI_OS_SPINLOCK` / `ATOMIC` 选择；AMP 下倾向 atomic。
- 禁止业务直接 `#include` `semphr.h` / `rthw.h`。

### 4.1 FreeRTOS 接管 SVC / PendSV 中断与 CubeMX 代码的冲突（STM32 实测）

**现象**：切到 `CONFIG_OS_FREERTOS` 后，链接期报 `multiple definition of 'SVC_Handler'` 和 `multiple definition of 'PendSV_Handler'`。
...
> 注意：mini_tree 仓库只提供 统一接口与 RTOS 内核，板级中断向量表 / `stm32f1xx_it.c` 由使用方的板级工程提供。该修复应在你自己的板级工程里做，不要回提到中间件。
...
| mini-os | `mini_os_schedule_start();`（`mini_scheduler_start` 内先惰性引导内核：`schedule_init` + idle 线程 + SysTick，再启动调度器） |
...
- **裸机队列池（仅 OS_BARE）**：`CONFIG_OS_BARE_MAX_QUEUES` 为**基础队列数**（默认 0，不占内存）；开启 `CONFIG_EVENT_BUS` 时**自动 +1**（EventBus 需要一个队列）。手动用 `mini_queue_create` → 在 Kconfig 设基础数。单队列缓冲 `CONFIG_OS_BARE_QUEUE_BUF_SZ`（默认 2048 B）。
- `CONFIG_OS_MUTEX_POOL_SIZE` 需覆盖 `DEV_ID_COUNT`（设备锁）及业务锁。
...
- [ ] 裸机任务创建路径符合预期 (`CONFIG_XTASK_PREEMPT`: 统一 C++ 重载 or 直接 xtask; `XTASK_NONE` 时强制关闭; 抢占式下重载带 `priority` 分支)
- [ ] 启动路径匹配后端
- [ ] 日志后端（PRINTF/ESP）仍符合预期
