...
The bare-metal backend (`CONFIG_OS_BARE`) picks one scheduler from the "bare-metal scheduler" choice in `Kconfig.mini_tree` (`XTASK_NONE` / `XTASK_COOP` / `XTASK_PREEMPT`); CMake injects `MINI_TREE_XTASK_*` macros to decide whether `xtask_coop.c` or `xtask_preempt.c` is compiled, with `#ifdef` as a second gate:
- **No scheduler** (`XTASK_NONE`) — nothing is compiled in; write your own `while(1)` loop. `CONFIG_XTASK_PREEMPT` is auto-disabled by Kconfig, and the core/system layer cannot link against xtask interfaces — the firmware degrades to a bare closure.
...
The public surface is `core/include/mini_backend.h`. Business code and VFS should depend on this header only.
...
1. Change the OS choice in `mini_tree/.config` (mutually exclusive).
...
- The bare-metal C API `mini_task_create` / `mini_task_create_handle` always returns `MINI_ERR_NOTSUPP`.

> **When `XTASK_PREEMPT=y`**: the C++ overload is still provided but switches to the `priority` branch (the cooperative/preemptive branches are split by `CONFIG_XTASK_PREEMPT` inside `（C++ 封装已移除）` — no caller change needed). You may also use `XTASK_NONE` and write a raw `while` loop.
...
- Only use APIs in `mini_backend.h` marked ISR-safe (if any); when in doubt, assume mutexes are **not** safe in ISRs.
- The spinlock implementation is selected by `CONFIG_MINI_OS_SPINLOCK` / `ATOMIC`; prefer atomic under AMP.
...
| mini-os | `mini_os_schedule_start();` (inside `mini_scheduler_start`, the kernel is lazily booted first: `schedule_init` + idle thread + SysTick, then the scheduler starts) |
...
- **Bare-metal queue pool (OS_BARE only)**: `CONFIG_OS_BARE_MAX_QUEUES` is the **base queue count** (default 0, no RAM); enabling `CONFIG_EVENT_BUS` **auto-adds 1** (EventBus needs a queue). Manual `mini_queue_create` → set the base in Kconfig. Per-queue buffer `CONFIG_OS_BARE_QUEUE_BUF_SZ` (2048 B).
- `CONFIG_OS_MUTEX_POOL_SIZE` must cover `DEV_ID_COUNT` (device locks) plus business locks.
...
- [ ] No two 后端 `.c` files compiled at once
...
- [ ] Bare-metal task-creation path is as intended (`CONFIG_XTASK_PREEMPT`: unified C++ overload or raw xtask; forced off under `XTASK_NONE`; preemptive overload has the `priority` branch)
- [ ] Startup path matches the backend
- [ ] Log backend (PRINTF/ESP) still behaves as expected
