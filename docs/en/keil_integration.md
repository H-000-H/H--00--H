...
4. **Cloud build**: use Keil Studio Cloud when no local toolchain is available; artifacts still align with `.config` (`CONFIG_ESP_*` / `CONFIG_SYSTEM_*`, etc.).
...
| Macros | Match `.config` (`CONFIG_ESP_*` / `CONFIG_SYSTEM_*` / `CONFIG_SYS_LOG_*` etc.) |
