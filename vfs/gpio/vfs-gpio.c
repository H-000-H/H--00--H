...
static int vfs_gpio_probe(struct device* pdev)
{
    ...
    if (device_get_prop_int(pdev, "gpio-port", &port_val) != MINI_OK || device_get_prop_int(pdev, "gpio-pin", &pin_val) != MINI_OK ||
        device_get_prop_int(pdev, "gpio-clk", &clk_val) != MINI_OK || device_get_prop_int(pdev, "gpio-mode", &mode_val) != MINI_OK ||
